...
- **Formatos de stream:** HLS (hls.js), MPEG-DASH (playback nativo del browser — **no usa dash.js**), HTML5 nativo (MP4, WebM, MP3)
...
window.loadMSPlayer('container-id', {
  ...
  id: 'content-id',
  autoplay: false,
  ads: { map: 'https://vast-server/tag' },  // forma canónica — player acepta ads.map
  // El QA Page Object acepta adsMap: string → harness remapea a ads.map automáticamente
}).then(player => { /* ... */ })
