...
### Paso 0 — Detectar modo de generación

Lee el contrato en `docs/05-pipeline/ai-test-generation/contract.md`.

Detecta el modo según el contexto de invocación:

**Modo Regresión** — existe `tmp/pipeline/coverage-report.json`:
- Input viene del pipeline (/review-diff)
- No requiere brief completo
- Cada test generado lleva `{ tag: ['@regression'] }`
- Cada spec lleva comentario: `// Draft: generado por pipeline — revisar antes de merge`

**Modo Feature** — no existe `tmp/pipeline/coverage-report.json`:
- Input viene de `docs/02-features/[feature]/` con brief completo
- Verificar presencia de: `feature-spec.md` (o `_meta.json`), `business-rules.md`, `observability.md`, `test-briefs.md`
- Si falta alguno → **no generar**, listar al usuario exactamente qué archivos faltan y detener

### Paso 1 — Leer context

Lee en este orden:
1. `tmp/pipeline/coverage-report.json` → extrae `specs_to_generate` con priority `MUST` (Modo Regresión)
   — O bien — `docs/02-features/[feature]/test-briefs.md` (Modo Feature)
2. `tmp/pipeline/risk-map.json` → extrae el diff y qué cambió exactamente (Modo Regresión)
